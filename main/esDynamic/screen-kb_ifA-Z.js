import{eM as O,gm as X,eN as j,r as _,eQ as R,a as v,f3 as V,f4 as U,eC as F,f2 as D,f5 as q,iT as B,eS as z,dq as n,eV as k,dp as i,dr as f,e_ as c,eU as u,f7 as G,eT as Q}from"./main-DclqjhjV.js";import{m as I}from"./marked.esm-DgqJBp_S.js";const Y=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Z={key:0},ee={class:"flex justify-center"},te={key:0,class:"flex flex-col justify-center text-center"},ae=["innerHTML"],le=["innerHTML"],se={key:3,class:"flex flex-col justify-center text-center"},oe={class:"text-xl mt-20"},re={key:1,class:"p-5"},ne=O({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(d){const s=X(),{t:r}=j(),x=U("iApi"),M=_(),a=d,w=R(()=>s.status),y=R(()=>s.response),P=_(!1),g=v({}),C=v([]),T=v([]);V(()=>{s.status="loading",S(),C.push(x.event.on(F.LAYER_REMOVE,e=>{a.payload.layer?.uid===e.uid&&a.panel.close()})),T.push(D(()=>a.payload.layer.uid,(e,t)=>{e!==t&&(s.status="loading",S())}))}),q(()=>{C.forEach(e=>x.event.off(e)),T.forEach(e=>e())});const S=()=>{P.value=a.payload.layer!==void 0&&!a.payload.layer.isRemoved,a.payload.type==="xml"?N(a.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(p(`${e.firstElementChild.outerHTML}`)),(a.payload.catalogueUrl||a.payload.url)&&t.appendChild(p(`<h5 class="text-xl font-bold mb-3">${r("metadata.xslt.metadata")}</h5>`)),a.payload.catalogueUrl&&t.appendChild(p(`<p><a style="color: blue;" href="${a.payload.catalogueUrl}" target="_blank">${r("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(p(`<p><a style="color: blue;" href="${a.payload.url}" target="_blank">${r("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):a.payload.type==="html"?b(a.payload.url).then(e=>{s.status=e.status,s.response=e.response}):a.payload.type==="md"&&b(a.payload.url).then(e=>{s.status=e.status,s.response=I(e.response)})},N=(e,t)=>{let l;return a.payload.xmlType&&a.payload.xmlType==="DCAT"?l=x.language==="en"?K:W:l=x.language==="en"?Y:J,l=l.replace(/\{\{([\w.]+)\}\}/g,(o,m)=>r(m)),g[e]?Promise.resolve($(g[e],l,t)):b(e).then(o=>(g[e]=o.response,$(g[e],l,t)))},$=(e,t,l)=>{let o=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,E=L.parseFromString(e,"text/xml"),H=L.parseFromString(t,"text/xml");m.importStylesheet(H),l&&l.forEach(h=>m.setParameter("",h.key,h.value||"")),o=m.transformToFragment(E,document);const A={className:"underline text-blue-700 break-all",target:"_blank",validate:{url:h=>/^https?:\/\//.test(h)}};a.payload.treatXmlAsMarkdown?o.firstChild.innerHTML=I.parse(o.firstChild.innerHTML):o.firstChild.innerHTML=B(o.firstChild.innerHTML,A)}return o},b=e=>new Promise(t=>{const l=new XMLHttpRequest;l.open("GET",e,!0),l.responseType="text",l.onload=()=>{l.status===200?t({status:"success",response:l.response}):t({status:"error",response:"Could not load results from remote service."})},l.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},l.send()});function p(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const l=Q("panel-screen");return n(),z(l,{panel:d.panel,ref_key:"el",ref:M},{header:k(()=>[G(c(u(r)("metadata.title"))+": "+c(d.payload.layerName),1)]),content:k(()=>[P.value?(n(),i("div",Z,[f("div",ee,[w.value=="loading"?(n(),i("div",te,c(u(r)("metadata.loading")),1)):d.payload.type==="xml"&&w.value=="success"?(n(),i("div",{key:1,innerHTML:y.value,class:"flex flex-col justify-center max-w-full xml-content"},null,8,ae)):(d.payload.type==="html"||d.payload.type==="md")&&w.value=="success"?(n(),i("div",{key:2,innerHTML:y.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,le)):(n(),i("div",se,[t[0]||(t[0]=f("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),f("span",oe,c(u(r)("metadata.error")),1)]))])])):(n(),i("div",re,[f("span",null,c(u(r)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{ne as default};
